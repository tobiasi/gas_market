...
with pd.ExcelWriter(output_filename, engine='xlsxwriter') as writer:
    # Sheet 1: Multiticker
    full_data_out.to_excel(writer, sheet_name='Multiticker', startrow=1, startcol=7)
    
    # Sheet 2: Demand
    countries_out.to_excel(writer, sheet_name='Demand', startrow=1, startcol=3)
    
    # Sheet 3: Demand YOY
    demand_out_final.to_excel(writer, sheet_name='Demand YOY', startrow=1, startcol=1)
    
    # Sheet 4: Supply
    supply_out.to_excel(writer, sheet_name='Supply', startrow=1, startcol=3)
    
    # Sheet 5: LNG imports by country
    lng_out.to_excel(writer, sheet_name='LNG imports by country', startrow=1, startcol=3)
    
    # Sheet 6: LDZ demand
    ldz_out.to_excel(writer, sheet_name='LDZ demand', startrow=1, startcol=3)
    
    # Sheet 7: Industrial demand
    industry_out.to_excel(writer, sheet_name='Industrial demand', startrow=1, startcol=3)
    
    # Sheet 8: Gas-to-Power demand
    gtp_out.to_excel(writer, sheet_name='Gas-to-Power demand', startrow=1, startcol=3)
    
    # Sheet 9: Calendar years - monthly levels (placeholder)
    Calendar_years_monthly_out.to_excel(writer, sheet_name='Calendar years - monthly levels', startrow=1, startcol=2)
    
    # Sheet 10: Calendar years - monthly
    Calendar_years_monthly_out.to_excel(writer, sheet_name='Calendar years - monthly', startrow=1, startcol=2)
    
    # Sheet 11: Calendar years, % - monthly
    Calendar_years_perc_monthly_out.to_excel(writer, sheet_name='Calendar years, % - monthly', startrow=1, startcol=2)
    
    # Sheet 12: Calendar years actuals
    Actuals_out.to_excel(writer, sheet_name='Calendar years actuals', startrow=1, startcol=2)
    
    # Sheet 13: Calendar years
    Calendar_years_out.to_excel(writer, sheet_name='Calendar years', startrow=1, startcol=2)
    
    # Sheet 14: Calendar years, %
    Calendar_years_perc_out.to_excel(writer, sheet_name='Calendar years, %', startrow=1, startcol=2)
    
    # Sheet 15: Calendar years YOY %
    Calendar_years_YOY_out.to_excel(writer, sheet_name='Calendar years YOY %', startrow=1, startcol=2)
    
    # Sheet 16: 2017-2021 average (placeholder)
    avg_2017_2021.to_excel(writer, sheet_name='2017-2021 average', startrow=1, startcol=2)
    
    # Sheet 17: Projected supply (placeholder)
    supply_out.to_excel(writer, sheet_name='Projected supply', startrow=1, startcol=3)
